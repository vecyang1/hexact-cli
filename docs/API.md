...
**Companion references** — this file is the curated, cross-checked reference;
two derived ones sit beside it:

- [`GATEWAY.md`](GATEWAY.md) — the full GraphQL schema recovered from validator
  error messages (all products, generated by `tools/schema_map.py`; do not
  hand-edit). Read it for exact field signatures the sections below only
  summarise.
- [`TYPES.md`](TYPES.md) — the *shape* of what each of those fields returns,
  recovered the same way (generated by `tools/type_map.py`; do not hand-edit).
  Read it before writing any query: GraphQL rejects a document that selects an
  object without naming its subfields, so a field whose return type is opaque
  cannot be called at all. Each entry carries a paste-ready leaf selection.
- [`REST_SURFACE.md`](REST_SURFACE.md) — every REST route proven to exist,
  method by method, with a credential-free route-existence oracle. Read it for
  what the shared router accepts and, just as usefully, what it does not.

Counts live in those generated files, not here. A number typed into prose is a
number that goes stale silently — this section said "27 query and 21 mutation
namespaces" for a while after the real figure had moved on.
...
One gateway carries the **whole suite** — every namespace, field, argument and
return type is enumerated in [`GATEWAY.md`](GATEWAY.md), and every return
type's shape in [`TYPES.md`](TYPES.md), both recovered with the "Did you mean"
oracle described below. Beyond `Watch*`
